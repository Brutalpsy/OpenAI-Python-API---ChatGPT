...
def main():
    ...
    if args.n not in range(1,150):
        raise ValueError("Error: n should be between 0 and 150")
...
def get_playlist(prompt, count=12):
    example_json = """
    [
...
def add_songs_to_spotify(playlist_prompt, playlist):
    ...
    for item in playlist:
        ...
        for query in [advanced_query, basic_query]:
            ...
            if not search_results["tracks"]["items"] or search_results["tracks"]["items"][0]["popularity"] < 5:
                continue
            else:
